"""
Evaluation wrapper for the CNN-Transformer energy forecasting model.
...
Interface required by evaluate.py:
    ...
    adapt_inputs(history_weather, history_energy, future_weather, future_time)
        ...
        Returns a tuple that is unpacked into forward().
...
ROOT = EVAL_DIR.parent.parent  # assignment3 project root on cluster

# When running from evaluation directory on cluster, the project code
...
def extract_calendar_features(hours_epoch):
    """
    Extract calendar features from hours-since-epoch tensor.

    Parameters
    ----------
    hours_epoch : (B, T) int64

    Returns
    -------
    torch.Tensor (B, T, 44) float32
    """
...
class EvalWrapper(nn.Module):
    """
    Wraps the trained CNN-Transformer model for the evaluation harness.

    Handles:
        - History subsampling (168h -> S hours)
        - Calendar feature extraction from timestamps
        - Weather/energy normalization
    ...
    def adapt_inputs(self, history_weather, history_energy,
        ...
        """
        Convert raw evaluation inputs to model inputs.
        ...
        # Subsample: take the most recent S hours from 168h history
        hist_weather = history_weather[:, -S:, :, :, :]
        hist_energy = history_energy[:, -S:, :]

        # Extract calendar features for history
        # Compute history hours from future_time
        # future_time[0] is the first prediction hour, history ends 1 hour before
        hist_start_hour = future_time[:, 0:1] - S  # (B, 1)
        hist_hours = hist_start_hour + torch.arange(S, device=future_time.device).unsqueeze(0)
        hist_cal = extract_calendar_features(hist_hours)

        # Calendar features for future
    ...
    def forward(self, hist_weather, hist_energy, hist_cal,
                future_weather, future_cal):
        """
        Run model and denormalize predictions.

        Returns
        -------
        (B, 24, n_zones) float32 in physical units (MWh)
        """
...
def get_model(metadata):
    """
    Build and return an EvalWrapper from dataset metadata.

    Parameters
    ----------
    metadata : dict
        From evaluate.py. Contains:
            n_zones, zone_names, history_len, future_len, n_weather_vars
    """
